...
kdr <- function(x, y, H, p = 1, max.iter = 400, tol.iter, segment = TRUE, k, kmax, min.seg.size, keep.path = FALSE, gridsize, xmin, xmax, binned,
    bgridsize, w, fhat, density.cutoff, pre = TRUE, verbose = FALSE) {
    ## default values
    xnames <- parse.name(x)
    x <- as.matrix(x)
    x.orig <- x
    ...
    ## default bandwidth
    if (missing(H))
        H <- Hpi(x = x, nstage = 2 - (d > 2), binned = binned, deriv.order = 2, verbose = verbose)
    ...
    if (is.vector(y))
        y <- matrix(y, nrow = 1)
    if (missing(min.seg.size))
        min.seg.size <- round(0.001 * nrow(y), 0)

    ## exclude low density regions from ridge search
    if (missing(fhat))
        fhat <- kde(x = x, w = w, binned = binned)
    if (missing(density.cutoff))
        density.cutoff <- contourLevels(fhat, cont = 99)
    ...
    pc <- kdr.base(x = x, fhat2 = fhat2, y = y[n.seq[i]:(n.seq[i + 1] - 1), ], H = H, tol.iter = tol.iter, Hinv = Hinv, verbose = verbose, max.iter = max.iter,
        p = p)
    ...
    if (length(n.seq) > 2) {
        for (i in 2:(length(n.seq) - 1)) {
            if (verbose)
                setTxtProgressBar(pb, i/(length(n.seq) - 1))
            pc.temp <- kdr.base(x = x, fhat2 = fhat2, y = y[n.seq[i]:(n.seq[i + 1] - 1), ], H = H, tol.iter = tol.iter, Hinv = Hinv, verbose = verbose,
                max.iter = max.iter, p = p)
    ...
    if (p == 1) {
        tol.seg <- 0.01 * max(apply(x.orig, 2, IQR))
    ...
    if (segment)
        pc <- kdr.segment(x = pc, k = k, kmax = kmax, min.seg.size = min.seg.size, verbose = verbose) else pc$end.points <- data.frame(pc$end.points, segment = 1L)
...
kdr.base <- function(x, fhat2, H, y, max.iter, tol.iter, p = 1, verbose = FALSE, Hinv, ...) {
    ...
    while (eps > tol.iter & i < max.iter) {
        ...
        if (verbose > 1) {
            if (d == 2)
                plot(y.update[disp.ind, ], col = 1, xlab = "x", ylab = "y") else pairs(y.update[disp.ind, ], col = 1)
        }
        i <- i + 1
    }
...
## create segment of KDR filaments x = output from kdr

kdr.segment <- function(x, k, kmax, min.seg.size, verbose = FALSE) {
    ep <- x$end.points
    if (any(names(ep) %in% "segment"))
        ep <- x$end.points[, -which(names(ep) == "segment")]
    if (missing(min.seg.size))
        min.seg.size <- x$min.seg.size
    ...
    ## re-order KDR segments into 'reasonable' linestring order experimental
    j <- 1
    for (i in unique(ep$segment)) {
        ...
        if (j == 1)
            ep.ord <- ep.temp else ep.ord <- rbind(ep.ord, ep.temp)
        j <- j + 1
    }
...
chain.knnx <- function(x, k1 = 1, k2 = 5) {
    ## concatenate the nearest neighbours in a chain start with first point in x
    if (!is.matrix(x))
        x <- as.matrix(x)
    d <- ncol(x)
    if (nrow(x) > 1) {
        ...
        ## decide which permutation is 'best' linestring break at max discontinuity
...
plot.kdr <- function(x, ...) {
    ...
    if (d == 2) {
        ...
        plotkdr.3d(fhat, ...)
        invisible()
    } else stop("Plot function only available for 2 or 3-d data")
}
...
plotkdr.2d <- function(x, add = FALSE, col, type = "p", alpha = 1, ...) {
    ...
    if (!any(names(xp) %in% "segment")) {
        ...
        if (!add)
            plot(xp, col = col, type = type, ...) else points(xp, col = col, ...)
    } else {
        xps <- unique(xp$segment)
        if (missing(col))
            col <- hcl.colors(n = length(xps), palette = "Set2")
        if (length(col) < length(xps))
            col <- rep(col, length(xps))
        col <- transparency.col(col, alpha = alpha)
        if (!add)
            plot(xp[, -ncol(xp)], col = "transparent", ...)
        for (i in 1:length(xps)) lines(xp[xp$segment == xps[i], -ncol(xp)], col = col[i], type = type, ...)
    }
}

plotkdr.3d <- function(x, display = "plot3D", colors, col, col.fun, alphavec, size = 3, cex = 1, pch = 1, theta = -30, phi = 40, d = 4, ticktype = "detailed",
    add = FALSE, xlab, ylab, zlab, alpha = 1, box = TRUE, axes = TRUE, type = "p", ...) {
    fhat <- x
    xp <- x$end.points
    ...
    if (disp %in% "plot3D") {
        if (!add)
            plot3D::scatter3D(x = xp[, 1], y = xp[, 2], z = xp[, 3], add = add, theta = theta, phi = phi, d = d, type = type, xlab = xlab, ylab = ylab,
                zlab = zlab, ticktype = ticktype, type = "n", col = NA, ...)
        for (i in 1:length(xps)) plot3D::scatter3D(x = xp[xp$segment == xps[i], 1], y = xp[xp$segment == xps[i], 2], z = xp[xp$segment == xps[i],
            3], cex = cex, col = col[i], add = TRUE, pch = pch, type = type, alpha = alpha, ...)
    } else if (disp %in% "rgl") {
        if (!requireNamespace("rgl", quietly = TRUE))
            stop("Install the rgl package as it is required.", call. = FALSE)
        for (i in 1:length(xps)) rgl::plot3d(x = xp[xp$segment == xps[i], 1], y = xp[xp$segment == xps[i], 2], z = xp[xp$segment == xps[i], 3], col = col[i],
            alpha = alpha, xlab = xlab, ylab = ylab, zlab = zlab, add = add | (i > 1), box = box, axes = axes, type = type, size = size, ...)
    }
}
