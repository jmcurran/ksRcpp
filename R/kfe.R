...
kfe <- function(x, G, deriv.order, inc = 1, binned, bin.par, bgridsize, deriv.vec = TRUE, add.index = TRUE, verbose = FALSE) {
    ...
    if (missing(binned))
        binned <- default.bflag(d, n)
    psir <- dmvnorm.deriv.sum(x = x, Sigma = G, deriv.order = r, inc = inc, binned = binned, bin.par = bin.par, bgridsize = bgridsize, deriv.vec = deriv.vec,
        verbose = verbose, kfe = TRUE, add.index = FALSE)

    if (add.index) {
        ind.mat <- dmvnorm.deriv(x = rep(0, d), mu = rep(0, d), Sigma = diag(d), deriv.order = r, only.index = TRUE)

        if (deriv.vec)
            return(list(psir = psir, deriv.ind = ind.mat)) else return(list(psir = psir, deriv.ind = unique(ind.mat)))
...
############################################################################### Plug-in unconstrained bandwidth for KFE Returns Plug-in
############################################################################### bandwidth
...
Hpi.kfe <- function(x, nstage = 2, pilot, pre = "sphere", Hstart, binned = FALSE, bgridsize, amise = FALSE, deriv.order = 0, verbose = FALSE, optim.fun = "optim") {
    if (deriv.order != 0)
        stop("Currently only deriv.order=0 is implemented")
    ...
    if (nstage == 2) {
        ...
        if (pilot1 == "dscalar") {
            ...
            ## gdscalar not used because it's an implicit computation without symmetriser matrices and is slower than direct computation with
        ...
        if (pilot1 == "dscalar")
            psi2.hat <- psins(r = r + 2, Sigma = var(x.star), deriv.vec = TRUE) else if (pilot1 == "dunconstr")
            psi2.hat <- psins(r = r + 2, Sigma = var(x), deriv.vec = TRUE)
    }

    if (pilot1 == "dscalar") {
        if (missing(Hstart))
            Hstart <- Gns(r = r, n = n, Sigma = var(x.star))
    } else if (pilot1 == "dunconstr") {
        if (missing(Hstart))
            Hstart <- Gns(r = r, n = n, Sigma = var(x))
    }
    ...
    if (!amise)
        return(H) else return(list(H = H, PI = amise.star))
}

############################################################################### Plug-in diagonal bandwidth for KFE Returns Plug-in bandwidth

Hpi.diag.kfe <- function(x, nstage = 2, pilot, pre = "scale", Hstart, binned = FALSE, bgridsize, amise = FALSE, deriv.order = 0, verbose = FALSE,
    optim.fun = "optim") {
    if (deriv.order != 0)
        stop("Currently only dervi.order=0 is implemented")
    ...
    if (pilot1 == "dunconstr")
        stop("Unconstrained pilot selectors are not suitable for Hpi.diag.kfe")
    ...
    if (nstage == 2) {
        ...
        psi2.hat <- kfe(x = x.star, G = H2, deriv.order = r + 2, add.index = FALSE, binned = binned, bgridsize = bgridsize, verbose = verbose)
    } else psi2.hat <- psins(r = r + 2, Sigma = var(x.star), deriv.vec = TRUE)
    ...
    if (missing(Hstart))
        Hstart <- Hns(x = x.star, deriv.order = r)
    Hstart <- matrix.sqrt(Hstart)
