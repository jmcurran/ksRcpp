...
kfs <- function(x, H, h, deriv.order = 2, gridsize, gridtype, xmin, xmax, supp = 3.7, eval.points, binned, bgridsize, positive = FALSE, adj.positive,
    w, verbose = FALSE, signif.level = 0.05) {
    r <- deriv.order
    ...
    if (d == 1) {
        if (missing(h))
            h <- hpi(x = x, nstage = 2, binned = default.bflag(d = d, n = n), deriv.order = r)

        ## KDDE for r=2
        fhatr <- kdde(x = x, h = h, deriv.order = r, gridsize = gridsize, gridtype = gridtype, xmin = xmin, xmax = xmax, supp = supp, eval.points = eval.points,
            binned = binned, bgridsize = bgridsize, w = w, deriv.vec = FALSE, verbose = verbose)

        ## KDE
        fhat <- kde(x = x, h = h, gridsize = gridsize, gridtype = gridtype, xmin = min(fhatr$eval.points), xmax = max(fhatr$eval.points), binned = binned,
            bgridsize = bgridsize, positive = positive, adj.positive = adj.positive, w = w)
        ...
        if (missing(H))
            H <- Hpi(x = x, nstage = 2 - (d > 2), binned = default.bflag(d = d, n = n), deriv.order = r, verbose = verbose)
        ...
        fhat <- kde(x = x, H = H, gridsize = gridsize, gridtype = gridtype, xmin = xmin, xmax = xmax, supp = supp, eval.points = eval.points, binned = binned,
            bgridsize = bgridsize, w = w, verbose = verbose)

        fhat.est <- as.vector(fhat$estimate)
        fhatr.est <- sapply(fhatr$estimate, as.vector)

        ## convert from vec to vech because vec'ed derivative contains repeated columns so its variance isn't invertible
        ...
        fhatr.eigen <- lapply(lapply(seq(1, nrow(fhatr.est)), function(i) {
            invvech(fhatr.est[i, ])
    ...
    if (num.test >= 1)
        num.test.seq <- c(1:num.test, rep(NA, prod(gs) - num.test)) else num.test.seq <- rep(NA, prod(gs))
    ...
    ## ESS = effective sample size ess <- n*fhat$estimate*dmvnorm.mixt(x=rep(0,d), mu=rep(0,d), Sigma=H, props=1) signif.ess <- ess >= 5
...
plot.kfs <- function(x, display = "filled.contour", col = 7, colors, abs.cont, alpha = 1, alphavec = 0.4, add = FALSE, ...) {
    ...
    if (is.vector(fhatr$H))
        d <- 1 else d <- ncol(fhatr$H)
    if (d == 1) {
