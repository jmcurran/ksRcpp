...
nu <- function(r, A) {
    ...
    ei <- eigen(A)$values
    tr.vec <- numeric(r)
    for (p in 1:r) tr.vec[p] <- sum(ei^p)
...
nu.rs <- function(r, s, A, B) {
    if (s == 0)
        nu.val <- nu(r = r, A = A) else if (s >= 1) {
        nu.val <- 0
        for (i in 0:r) for (j in 0:(s - 1)) nu.val <- nu.val + choose(r, i) * choose(s - 1, j) * factorial(r + s - i - j - 1) * 2^(r + s - i - j -
            1) * tr(matrix.pow(A, r - i) %*% matrix.pow(B, s - j)) * nu.rs(r = i, s = j, A = A, B = B)
    }
...
gamma.r <- function(mu, Sigma, r) {
    ...
    d <- ncol(Sigma)
    v <- 0
    for (j in 0:r) v <- v + (-1)^j * choose(2 * r, 2 * j) * OF(2 * j) * nu.rs(r = r - j, s = j, A = Sigmainv %*% mu %*% t(mu) %*% Sigmainv, B = Sigmainv)
...
gamma.r2 <- function(mu, Sigma, d, r, H) {
    ...
    if (d == 1)
        w <- vec(Kpow(Sigmainv %*% Sigmainv, r)) %x% vec(Kpow(Sigmainv %*% H %*% Sigmainv, 2)) else w <- matrix(Sdrv(d = d, r = 2 * r + 4, v = vec(Kpow(Sigmainv %*% Sigmainv, r)) %x% vec(Kpow(Sigmainv %*% H %*% Sigmainv, 2))), nrow = 1)

    v <- rep(0, length = d^(2 * r + 4))
    for (j in 0:(r + 2)) v <- v + ((-1)^j * OF(2 * j) * choose(2 * r + 4, 2 * j)) * (Kpow(mu, 2 * r - 2 * j + 4) %x% Kpow(vec(Sigma), j))
...
############################################################################### Omega matrices (for exact MISE for normal mixtures) Parameters
############################################################################### mus - means Sigmas - variances k - number of mixture components
############################################################################### a - subscript of Omega matrix H - bandwidth matrix Returns Omega
############################################################################### matrix

omega <- function(mus, Sigmas, k, a, H, r) {
    ## the (i,j) element of Omega matrix is dmvnorm(0, mu_i - mu_j, a*H + Sigma_i + Sigma_j)
    if (is.matrix(mus))
        d <- ncol(mus) else d <- length(mus)

    if (k == 1)
        omega.mat <- gamma.r(mu = rep(0, d), Sigma = a * H + 2 * Sigmas, r = r) else {
...
omega.1d <- function(mus, sigmas, k, a, h, r) {
    ...
    if (k == 1)
        omega.mat <- gamma.r(mu = 0, Sigma = as.matrix(a * H + 2 * Sigmas), r = r) else {
        omega.mat <- matrix(0, nrow = k, ncol = k)
        for (i in 1:k) {
            Sigmai <- Sigmas[i]
            mui <- mus[i]
...
############################################################################## Exact MISE for normal mixtures Parameters mus - means Sigmas -
############################################################################## variances Props - vector of proportions of each mixture component
############################################################################## H - bandwidth matrix samp - sample size Returns Exact MISE for
############################################################################## normal mixtures
...
mise.mixt <- function(H, mus, Sigmas, props, samp, h, sigmas, deriv.order = 0) {
    ...
    if (k == 1) {
        mise <- 2^(-r) * nu(r, Hinv)/(samp * (4 * pi)^(d/2) * sqrt(det(H))) + (1 - 1/samp) * omega(mus, Sigmas, 1, 2, H, r) - 2 * omega(mus, Sigmas,
            1, 1, H, r) + omega(mus, Sigmas, 1, 0, H, r)
    } else {
        mise <- 2^(-r) * nu(r, Hinv)/(samp * (4 * pi)^(d/2) * sqrt(det(H))) + props %*% ((1 - 1/samp) * omega(mus, Sigmas, k, 2, H, r) - 2 * omega(mus,
            Sigmas, k, 1, H, r) + omega(mus, Sigmas, k, 0, H, r)) %*% props
    }
...
mise.mixt.1d <- function(h, mus, sigmas, props, samp, deriv.order = 0) {
    ...
    ## formula is found in Wand & Jones (1993) and Chacon, Duong & Wand (2008)
    if (k == 1) {
        mise <- 2^(-r) * nu(r, Hinv)/(samp * (4 * pi)^(d/2) * sqrt(det(H))) + (1 - 1/samp) * omega.1d(mus, sigmas, 1, 2, h, r) - 2 * omega.1d(mus,
            sigmas, 1, 1, h, r) + omega.1d(mus, sigmas, 1, 0, h, r)
    } else {
        mise <- 2^(-r) * nu(r, Hinv)/(samp * (4 * pi)^(d/2) * sqrt(det(H))) + props %*% ((1 - 1/samp) * omega.1d(mus, sigmas, k, 2, h, r) - 2 * omega.1d(mus,
            sigmas, k, 1, h, r) + omega.1d(mus, sigmas, k, 0, h, r)) %*% props
    }

    return(drop(mise))
}

############################################################################### Exact AMISE for bivariate normal mixtures Parameters mus - means
############################################################################### Sigmas - variances props - mixing proportions H - bandwidth
############################################################################### matrix samp - sample size Returns Exact AMISE for normal
############################################################################### mixtures
...
amise.mixt <- function(H, mus, Sigmas, props, samp, h, sigmas, deriv.order = 0) {
    ...
    Hinv <- chol2inv(chol(H))
    if (k == 1)
        amise <- 2^(-r) * nu(r, Hinv)/(samp * (4 * pi)^(d/2) * sqrt(det(H))) + omega.mat/4 else amise <- 2^(-r) * nu(r, Hinv)/(samp * (4 * pi)^(d/2) * sqrt(det(H))) + (props %*% omega.mat %*% props)/4
...
amise.mixt.1d <- function(h, mus, sigmas, props, samp, deriv.order = 0) {
    ...
    if (k == 1)
        omega.mat <- gamma.r2(mu = rep(0, d), Sigma = as.matrix(2 * sigmas^2), d = d, r = r, H = H) else {
        omega.mat <- matrix(0, nrow = k, ncol = k)
        for (i in 1:k) {
...
############################################################################### Lambda matrices (for exact AMISE for normal mixtures) Parameters
############################################################################### mus - means Sigmas - variances k - number of mixture components
############################################################################### r - derivative (r1, r2) Returns Lambda matrix

lambda <- function(mus, Sigmas, k, r) {
    ## the (i,j) element of Lambda matrix is d^r/ dx^r dmvnorm(0, mu_i - mu_j, a*H + Sigma_i + Sigma_j)

    if (is.vector(mus))
        d <- length(mus) else d <- ncol(mus)

    if (k == 1)
        lambda.mat <- dmvnorm.deriv(deriv.order = r, x = rep(0, length(mus)), Sigma = 2 * Sigmas) else {
        if (is.matrix(mus))
            d <- ncol(mus) else d <- length(mus)
...
amise.mixt.2d <- function(H, mus, Sigmas, props, samp) {
    ...
    if (k == 1) {
        amise <- 1/(samp * (4 * pi)^(d/2) * sqrt(det(H))) + 1/4 * (lambda(mus, Sigmas, k, r = c(4, 0)) * H[1, 1]^2 + 4 * lambda(mus, Sigmas, k, r = c(3,
            1)) * H[1, 1] * H[1, 2] + 2 * lambda(mus, Sigmas, k, r = c(2, 2)) * (H[1, 1] * H[2, 2] + 2 * H[1, 2]^2) + 4 * lambda(mus, Sigmas, k, r = c(1,
            3)) * H[2, 2] * H[1, 2] + lambda(mus, Sigmas, k, r = c(0, 4)) * H[2, 2]^2)
    } else {
        amise <- 1/(samp * (4 * pi)^(d/2) * sqrt(det(H))) + 1/4 * props %*% (lambda(mus, Sigmas, k, r = c(4, 0)) * H[1, 1]^2 + 4 * lambda(mus, Sigmas,
            k, r = c(3, 1)) * H[1, 1] * H[1, 2] + 2 * lambda(mus, Sigmas, k, r = c(2, 2)) * (H[1, 1] * H[2, 2] + 2 * H[1, 2]^2) + 4 * lambda(mus,
            Sigmas, k, r = c(1, 3)) * H[2, 2] * H[1, 2] + lambda(mus, Sigmas, k, r = c(0, 4)) * H[2, 2]^2) %*% props
    }

    return(drop(amise))
}

############################################################################### Finds the bandwidth matrix that minimises the MISE for normal
############################################################################### mixtures Parameters mus - means Sigmas - variances props -
############################################################################### vector of proportions of each mixture component Hstart - initial
############################################################################### bandwidth matrix samp - sample size full - 1 minimise over full
############################################################################### bandwidth matrices - 0 minimise over diagonal bandwidth matrices
############################################################################### Returns H_MISE
...
############################################################################### Finds bandwidth matrix that minimises the AMISE for normal
############################################################################### mixtures - 2-dim Parameters mus - means Sigmas - variances props
############################################################################### - vector of proportions of each mixture component Hstart -
############################################################################### initial bandwidth matrix samp - sample size Returns Bandwidth
############################################################################### matrix that minimises AMISE
...
############################################################################### ISE for normal mixtures (fixed KDE) Parameters x - data values H
############################################################################### - bandwidth matrix mus - matrix of means (each row is a vector
############################################################################### of means from each component density) Sigmas - matrix of
############################################################################### covariance matrices (every d rows is a covariance matrix from
############################################################################### each component density) props - mixing proportions lower -
############################################################################### vector of lower end points of rectangle upper - vector of upper
############################################################################### end points of rectangle gridsize - vector of number of grid
############################################################################### points stepsize - vector of step sizes Returns ISE
...
ise.mixt <- function(x, H, mus, Sigmas, props, h, sigmas, deriv.order = 0, binned = FALSE, bgridsize) {
    ...
    if (is.vector(x))
        x <- matrix(x, nrow = 1)
    if (is.vector(mus))
        mus <- matrix(mus, nrow = length(props))
