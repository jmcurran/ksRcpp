...
ksupp <- function(fhat, cont = 95, abs.cont, convex.hull = TRUE) {
    if (missing(abs.cont))
        abs.cont <- contourLevels(fhat, cont = cont)
    supp <- expand.grid(fhat$eval.points)[as.vector(fhat$estimate > abs.cont), ]
    d <- length(fhat$eval.points)
    if (d == 2) {
        if (convex.hull)
            supp <- supp[chull(supp), ]
    } else {
        if (convex.hull) {
            if (!requireNamespace("geometry", quietly = TRUE))
                stop("Install the geometry package as it is required.", call. = FALSE)
...
dwsupp <- function(x, H, h, gridsize, gridtype, xmin, xmax, supp = 3.7, binned, bgridsize, verbose = FALSE, w) {
    ...
    if (d == 1) {
        y <- x
        if (missing(h))
            h <- hpi(x = y, binned = default.bflag(d = d, n = n), bgridsize = bgridsize)
    }
    if (missing(H) & d > 1) {
    ...
    for (i in 1:n) {
        ...
        for (j in 1:length(eval.y)) fhat.grid[eval.x.ind, eval.y.ind[j]] <- fhat.grid[eval.x.ind, eval.y.ind[j]] + w[i] * fhat[((j - 1) * eval.x.len +
            1):(j * eval.x.len)]
        if (verbose)
...
plot.ksupp <- function(x, display = "plot3D", ...) {
    d <- ncol(x)
    if (d == 2)
        polygon(x, ...) else if (d == 3) {
        disp <- match.arg(display, c("plot3D", "rgl"))
        if (disp == "plot3D")
            plot3D::triangle3D(x, ...) else if (disp == "rgl") {
