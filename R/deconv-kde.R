...
kdcde <- function(x, H, h, Sigma, sigma, reg, bgridsize, gridsize, binned, verbose = FALSE, ...) {
    ...
    x <- as.matrix(x)
    if (d == 1 & missing(h))
        h <- hpi(x = x, nstage = 2, binned = binned, deriv.order = 0)
    if (d > 1 & missing(H))
        H <- Hpi(x = x, nstage = 2, binned = binned, deriv.order = 0)

    if (d == 1)
        stop("dckde not yet implemented for d=1")
    if (missing(reg))
        reg <- reg.ucv(x = x, H = H, h = h, Sigma = Sigma, sigma = sigma, k = 5, d = d, binned = binned, verbose = verbose)
...
## weights for deconvolution KDE code adapted from DeconWK 0.6-5, author B. Turlach R-forge website:
## https://r-forge.r-project.org/R/?group_id=630
