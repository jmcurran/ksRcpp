...
ks.defaults <- function(x, w, binned, bgridsize, gridsize) {
    ...
    if (missing(bgridsize)) {
        if (missing(gridsize))
            bgridsize <- default.bgridsize(d) else bgridsize <- gridsize
    }
    if (missing(gridsize))
        gridsize <- default.gridsize(d)
    if (length(gridsize) == 1)
        gridsize <- rep(gridsize, d)
    if (length(bgridsize) == 1)
        bgridsize <- rep(bgridsize, d)
...
vech <- function(x) {
    if (is.vector(x)) {
        if (length(x) == 1)
            return(x) else stop("vech undefined for vectors")
    } else if (is.matrix(x)) {
        d <- ncol(x)
        if (d != nrow(x))
...
invvec <- function(x, ncol, nrow, byrow = FALSE) {
    ...
    if (byrow)
        for (j in 1:nrow) invvecx[j, ] <- x[c(1:ncol) + (j - 1) * ncol] else for (j in 1:ncol) invvecx[, j] <- x[c(1:nrow) + (j - 1) * nrow]
...
invvech <- function(x) {
    ...
    for (j in 1:d) invvechx[j:d, j] <- x[1:(d - j + 1) + (j - 1) * (d - 1/2 * (j - 2))]
...
tr <- function(A) {
    ...
    if (is.vector(A))
        return(A[1])
    if (nrow(A) != ncol(A))
        stop("Not square matrix") else for (i in 1:nrow(A)) count <- count + A[i, i]
...
########################################################################## Duplication matrix Taken from Felipe Osorio
########################################################################## http://www.ime.usp.br/~osorio/files/dupl.q
...
########################################################################## Pre-scaling Parameters x - data points Returns Pre-scaled x values
...
pre.scale <- function(x, mean.centred = FALSE) {
    ...
    if (mean.centred)
        x.scaled <- sweep(x, 2, apply(x, 2, mean)) else x.scaled <- x
    x.scaled <- x.scaled %*% Sinv12

    return(x.scaled)
}

########################################################################## Pre-sphering Parameters x - data points Returns Pre-sphered x values
...
pre.sphere <- function(x, mean.centred = FALSE) {
    ...
    if (mean.centred)
        x.sphered <- sweep(x, 2, apply(x, 2, mean)) else x.sphered <- x
...
########################################################################## Finds row index matrix Parameters x - data points Returns i - if
########################################################################## r==mat[i,] NA - otherwise

which.mat <- function(r, mat) {
    ind <- numeric()

    for (i in 1:nrow(mat)) if (identical(r, mat[i, ]))
        ind <- c(ind, i)
...
########################################################################## Permutations with repetitions of the first d naturals (1:d) taking k
########################################################################## elements at a time. There are d^k of them, each having length k => We
########################################################################## arrange them into a matrix of order d^k times k Each row represents
########################################################################## one permutation Second version: filling in the matrix comlumn-wise
########################################################################## (slightly faster)
...
########################################################################## Permute a list of values Same function as EXPAND.GRID (base package),
########################################################################## modified to take list as an argument and returns a matrix
...
block.indices <- function(nx, ny, d, r = 0, diff = FALSE, block.limit = 1e+06, npergroup) {
    if (missing(npergroup)) {
        if (diff)
            npergroup <- max(c(block.limit%/%(nx * d^r), 1)) else npergroup <- max(c(block.limit%/%nx, 1))
    }
...
block.indices2 <- function(nx, ny, block.limit = 1e+06, npergroup) {
    if (missing(npergroup))
        npergroup <- max(c(block.limit%/%nx, 1))
    nseq <- seq(1, ny, by = npergroup)
    if (tail(nseq, n = 1) <= ny)
        nseq <- c(nseq, ny + 1)
    if (length(nseq) == 1)
...
differences <- function(x, y, upper = FALSE, ff = FALSE, Kpow = 0) {
    ...
    if (ff)
        difs <- ff(init = 0, dim = c(nx * ny, d)) else difs <- matrix(ncol = d, nrow = nx * ny)
    ...
    if (upper) {
        ind.remove <- numeric()
        for (j in 1:(nx - 1)) ind.remove <- c(ind.remove, (j * nx + 1):(j * nx + j))
...
OF <- function(m) {
    factorial(m)/(2^(m/2) * factorial(m/2))
}
...
matrix.sqrt <- function(A) {
    ...
    sva <- svd(A)
    if (min(sva$d) >= 0)
        Asqrt <- sva$u %*% diag(sqrt(sva$d)) %*% t(sva$v) else stop("Matrix square root is not defined")
...
matrix.pow <- function(A, n) {
    ...
    if (n == 0)
        return(diag(ncol(A)))
    if (n < 0)
        return(matrix.pow(A = chol2inv(chol(A)), n = -n))
...
########################################################################## mat.Kprod computes row-wise Kronecker products of matrices Returns a
########################################################################## matrix with rows U[i,]%x%V[i,]

mat.Kprod <- function(U, V) {
    n1 <- nrow(U)
    n2 <- nrow(V)
    if (n1 != n2)
        stop("U and V must have the same number of vectors")
    p <- ncol(U)
    q <- ncol(V)
    onep <- rep(1, p)
    oneq <- rep(1, q)
...
Ksum <- function(A, B) {
    AB <- numeric()
    for (i in 1:nrow(A)) for (j in 1:nrow(B)) AB <- rbind(AB, A[i, ] + B[j, ])
...
rowKpow <- function(A, B, r = 1, s = 1) {
    ...
    if (missing(B)) {
        ...
        B <- as.matrix(B)
        if (nrow(A) != nrow(B))
            stop("A and B must have same number of rows")
        res <- numeric()
...
mat.Kpow <- function(A, pow) {
    Apow <- A
    if (pow == 0) {
        Apow <- matrix(1, nrow = nrow(A), ncol = 1)
    }
    if (pow > 1) {
...
########################################################################## Sdr.direct computes the symmetrizer matrix S_{d,r} based on Equation
########################################################################## (4) as described in Section 3 of Chacon and Duong (2014)
...
########################################################################## Sdr.recursive computes the symmetrizer matrix S_{d,r} based on the
########################################################################## recursive approach detailed in Algorithm 1 in Section 3 of Chacon and
########################################################################## Duong (2014)
...
########################################################################## S_{d,r} by a vector v of length d^r, based on Equation (4) as
########################################################################## described in Section 4 of Chacon and Duong (2014)
...
########################################################################## Sdrv.recursive computes the result of multiplying the symmetrizer
########################################################################## matrix S_{d,r} by a vector v of length d^r, based on the recursive
########################################################################## Algorithm 2 as described in Section 4 of Chacon and Duong (2014)

Sdrv.recursive <- function(d, r, v) {
    if ((!is.vector(v)) & (!is.matrix(v))) {
        stop("v must be a vector or a matrix")
    }
    if (is.vector(v)) {
        v <- matrix(v, nrow = 1)
    }
    n <- nrow(v)
    if (ncol(v) != d^r) {
        stop("Length of the vector(s) must equal d^r")
    }
...
Lpdiff <- function(f1, f2, p = 2, index = 1) {
    if (is.vector(f1$H))
        d <- 1 else d <- ncol(f1$H)
    f.diff <- f1
    if (is.list(f1$estimate))
        f.diff$estimate <- (abs(f1$estimate[[index]] - f2$estimate[[index]]))^p else f.diff$estimate <- (abs(f1$estimate - f2$estimate))^p
    if (d == 1) {
        ...
        if (d == 2)
            riemann.sum <- sum(outer(delta[, 1], delta[, 2]) * f.diff$estimate) else if (d == 3)
            riemann.sum <- sum(outer(outer(delta[, 1], delta[, 2]), delta[, 3]) * f.diff$estimate)
    }
...
ise.diff <- function(fhat1, fhat2, xmin, xmax) {
    ...
    if (!missing(xmin) & missing(xmax))
        int <- integral.kde(fhat = fhat.sq, q = max(fhat.sq$eval.points) + 0.1 * abs(max(fhat.sq$eval.points)), density = FALSE) - integral.kde(fhat = fhat.sq,
            q = xmin, density = FALSE)
...
########################################################################## Create transparent colour partial alias for plot3D::alpha.col
