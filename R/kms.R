...
kms <- function(x, y, H, max.iter = 400, tol.iter, tol.clust, min.clust.size, merge = TRUE, keep.path = FALSE, verbose = FALSE) {
    ...
    d <- ncol(x)
    if (missing(tol.iter))
        tol.iter <- 0.001 * min(apply(x, 2, IQR))  ## mean(apply(apply(x, 2, range), 2, diff))
    if (missing(tol.clust))
        tol.clust <- 0.01 * max(apply(x, 2, IQR))  ## mean(apply(apply(x, 2, range), 2, diff))  
    if (missing(y))
        y <- x
    ...
    ms <- kms.base(x = x, y = y[n.seq[i]:(n.seq[i + 1] - 1), ], H = H, tol.iter = tol.iter, tol.clust = tol.clust, Hinv = Hinv, verbose = verbose,
        max.iter = max.iter)
    ...
    if (length(n.seq) > 2) {
        for (i in 2:(length(n.seq) - 1)) {
            if (verbose)
                setTxtProgressBar(pb, i/(length(n.seq) - 1))
            ms.temp <- kms.base(x = x, y = y[n.seq[i]:(n.seq[i + 1] - 1), ], H = H, tol.iter = tol.iter, tol.clust = tol.clust, Hinv = Hinv, verbose = verbose,
                max.iter = max.iter)
    ...
    if (merge)
        ms <- ms.merge.num(ms, min.clust.size = min.clust.size, verbose = verbose)

    return(ms)
}

kms.base <- function(x, H, Hinv, y, max.iter, tol.iter, tol.clust, verbose = FALSE) {
    ## mean shift iterations original implementation J.E. Chacon (2013) modifications T.D. (2014)
    ...
    while (eps > tol.iter & i < max.iter) {
        ...
        if (verbose > 1) {
            ...
            if (d == 2)
                plot(y.update[disp.ind, ], col = 1, xlim = y.range[, 1], ylim = y.range[, 2], xlab = "x", ylab = "y") else pairs(y.update[disp.ind, ], col = 1)
        }
        i <- i + 1
    }
...
## merge classes in 'label' into a single class label is a list of vector of class labels i.e. all classes in label[[j]] merged into new class j
...
ms.merge.dist <- function(ms, tol, verbose) {
    ...
    ## create list where each element is a vector of cluster labels to be merged into a single cluster
...
ms.merge.num <- function(ms, min.clust.size, verbose = FALSE) {
    ...
    if (any(ms$nclust.table <= min.clust.size)) {
        if (verbose)
            cat("Min cluster size merging begins. Min size = ", min.clust.size, "\n")
...
kms.part <- function(x, H, xmin, xmax, gridsize, verbose = FALSE, ...) {
    if (missing(H))
        H <- Hpi(x, deriv.order = 1, binned = TRUE)
    tol <- 5
    tol.H <- tol * diag(H)
    if (missing(xmin))
...
plot.kde.part <- function(x, display = "filled.contour", col, col.fun, alpha = 1, add = FALSE, ...) {
    ...
    if (missing(col.fun))
        col.fun <- function(n) {
            hcl.colors(n, palette = "Dark2", alpha = alpha)
        }
    if (missing(col))
        col <- col.fun(length(clev))
...
plot.kms <- function(x, display = "splom", col, col.fun, alpha = 1, xlab, ylab, zlab, theta = -30, phi = 40, add = FALSE, ...) {
    ...
    if (missing(col.fun))
        col.fun <- function(n) {
            hcl.colors(n, palette = "Set2", alpha = alpha)
        }
    if (missing(col))
        col <- col.fun(length(unique(x$label)))
    col <- transparency.col(col, alpha = alpha)
    if (d == 1)
        stop("kms plot not yet implemented") else if (d == 2) {
        if (missing(xlab))
            xlab <- x$names[1]
        if (missing(ylab))
            ylab <- x$names[2]
        if (!add)
            plot(x$x, col = col[x$label], xlab = xlab, ylab = ylab, ...) else points(x$x, col = col[x$label], ...)
        ...
        if (disp == "plot3D") {
            if (!add)
                plot3D::points3D(x$x[, 1], x$x[, 2], x$x[, 3], col = 1, cex = 0, add = add, theta = theta, phi = phi, d = 4, colkey = FALSE, xlab = xlab,
                  ylab = ylab, zlab = zlab, ticktype = "detailed", bty = "f", ...)

            for (i in 1:length(col)) plot3D::points3D(x$x[x$label == i, 1], x$x[x$label == i, 2], x$x[x$label == i, 3], col = col[i], add = TRUE,
                ...)
        } else if (disp == "rgl") {
            ...
            if (!add)
                rgl::plot3d(x$x, col = col[x$label], alpha = alpha, ...) else rgl::points3d(x$x, col = col[x$label], alpha = alpha, ...)
        }
    } else if (d >= 3) {
